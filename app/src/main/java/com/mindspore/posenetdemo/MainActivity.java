...
public class MainActivity extends AppCompatActivity implements CameraDataDealListener {
     ...
     * Draw bitmap on Canvas.
     */
    private void sendData(final Posenet.Person person) {
...
//                Log.w("TAG", "run: sendData Function 1");

                MediaType JSON = MediaType.parse("application/json; charset=utf-8");
                JSONObject json = new JSONObject();

                try {
                    json.put("test", "android");
                    json.put("score", person.score);
                    json.put("time", System.currentTimeMillis());
                    JSONObject keyPoints = new JSONObject();
                    for(Posenet.BodyPart part : Posenet.BodyPart.values())
                    {
                        Posenet.KeyPoint keyPoint = person.keyPoints.get(part.ordinal());
                        JSONObject position = new JSONObject();
                        position.put("x", keyPoint.position.x);
                        position.put("y", keyPoint.position.y);
                        JSONObject bodyPart = new JSONObject();
                        bodyPart.put("score", keyPoint.score);
                        bodyPart.put("position", position);
//                        Log.w("TAG", "run: sendData " + part.name() + " " + keyPoint.position.x + " " + keyPoint.position.y + " " + System.currentTimeMillis());
                        keyPoints.put(part.name(), bodyPart);
                    }
                    json.put("keyPoints", keyPoints);
...
//                Log.w("TAG", "run: sendData Function 3");
                try {
